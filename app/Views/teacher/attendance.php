...
<style>
    ...
    .status-btn:hover { transform: translateY(-1px); box-shadow: 0 2px 8px rgba(0,0,0,0.15); }
    .status-btn.btn-hadir { background:#ecfdf5; color:#059669; border-color:#a7f3d0; }
    .status-btn.btn-hadir.active,.status-btn.btn-hadir:hover { background:#059669; color:white; border-color:#059669; }
    .status-btn.btn-sakit { background:#fff7ed; color:#ea580c; border-color:#fed7aa; }
    .status-btn.btn-sakit.active,.status-btn.btn-sakit:hover { background:#ea580c; color:white; border-color:#ea580c; }
    .status-btn.btn-izin  { background:#eff6ff; color:#2563eb; border-color:#bfdbfe; }
    .status-btn.btn-izin.active,.status-btn.btn-izin:hover   { background:#2563eb; color:white; border-color:#2563eb; }
    .status-btn.btn-alpa  { background:#fef2f2; color:#dc2626; border-color:#fecaca; }
    .status-btn.btn-alpa.active,.status-btn.btn-alpa:hover   { background:#dc2626; color:white; border-color:#dc2626; }
    ...
    .student-card::before {
        ...
        width:4px; height:100%; background:#e5e7eb; transition:background 0.3s ease;
    }
    .student-card.status-hadir { border-color:#a7f3d0; }
    .student-card.status-hadir::before { background:#059669; }
    .student-card.status-sakit { border-color:#fed7aa; }
    .student-card.status-sakit::before { background:#ea580c; }
    .student-card.status-izin  { border-color:#bfdbfe; }
    .student-card.status-izin::before  { background:#2563eb; }
    .student-card.status-alpha { border-color:#fecaca; }
    .student-card.status-alpha::before { background:#dc2626; }

    .jam-row { display:flex; gap:8px; margin-top:10px; margin-bottom:6px; }
    .jam-item { flex:1; background:#f9fafb; border-radius:10px; padding:8px 10px; text-align:center; border:1px solid #e5e7eb; }
    .jam-item label { display:block; font-size:10px; font-weight:600; color:#6b7280; text-transform:uppercase; letter-spacing:.5px; margin-bottom:4px; }
    .jam-item .jam-value { font-size:14px; font-weight:700; color:#1f2937; }
    .jam-item .jam-value.empty { color:#9ca3af; }
    ...
    .stat-card { border-radius:16px; padding:20px 24px; display:flex; align-items:center; gap:16px; }
    .stat-card .stat-icon { width:48px; height:48px; border-radius:12px; display:flex; align-items:center; justify-content:center; }

    .submit-btn { background:linear-gradient(135deg,#059669,#047857); color:white; padding:10px 24px; border-radius:12px; font-weight:600; border:none; cursor:pointer; display:flex; align-items:center; gap:8px; transition:all 0.2s ease; box-shadow:0 4px 12px rgba(5,150,105,.3); }
    .submit-btn:hover { transform:translateY(-1px); box-shadow:0 6px 16px rgba(5,150,105,.4); }
    .submit-btn:disabled { opacity:.6; cursor:not-allowed; transform:none; }

    .mark-all-btn { padding:8px 16px; border-radius:10px; font-weight:500; font-size:13px; cursor:pointer; border:2px solid; transition:all 0.2s ease; }

    .lock-banner { background:linear-gradient(135deg,#fef3c7,#fde68a); border:2px solid #f59e0b; border-radius:16px; padding:16px 24px; display:flex; align-items:center; gap:16px; margin-bottom:24px; }
    .lock-banner .lock-icon { width:48px; height:48px; background:#f59e0b; border-radius:12px; display:flex; align-items:center; justify-content:center; flex-shrink:0; }
    .lock-banner .lock-icon span { color:white; font-size:24px; }
    .page-locked .student-card, .page-locked .mark-all-btn, .page-locked .submit-btn { opacity:.5; pointer-events:none; }

    .date-nav-btn { width:42px; height:42px; border-radius:12px; border:2px solid #e5e7eb; background:white; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:all 0.2s ease; flex-shrink:0; }
    .date-nav-btn:hover { border-color:#6366f1; background:#eef2ff; }
    .date-display { min-width:200px; text-align:center; cursor:pointer; padding:8px 16px; border-radius:12px; border:2px solid #e5e7eb; background:white; position:relative; }
    .date-display:hover { border-color:#6366f1; }
    #datePickerInput { position:absolute; opacity:0; width:100%; height:100%; top:0; left:0; cursor:pointer; }
</style>
<?= $this->endSection() ?>
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <div class="stat-card bg-white border border-gray-200 shadow-sm">
        ...
        <div><p class="text-sm text-gray-500">Total Siswa</p><p class="text-2xl font-bold text-gray-900" id="statTotal">0</p></div>
    </div>
    <div class="stat-card bg-white border border-gray-200 shadow-sm">
        <div class="stat-icon bg-green-100"><span class="material-symbols-outlined text-green-600">how_to_reg</span></div>
        <div><p class="text-sm text-gray-500">Hadir</p><p class="text-2xl font-bold text-green-600" id="statHadir">0</p></div>
    </div>
    <div class="stat-card bg-white border border-gray-200 shadow-sm">
        <div class="stat-icon bg-yellow-100"><span class="material-symbols-outlined text-yellow-600">medical_services</span></div>
        <div><p class="text-sm text-gray-500">Sakit / Izin</p><p class="text-2xl font-bold text-yellow-600" id="statSakitIzin">0</p></div>
    </div>
    <div class="stat-card bg-white border border-gray-200 shadow-sm">
        <div class="stat-icon bg-red-100"><span class="material-symbols-outlined text-red-600">person_off</span></div>
        <div><p class="text-sm text-gray-500">Alpa</p><p class="text-2xl font-bold text-red-600" id="statAlpa">0</p></div>
    </div>
</div>
...
<script>
    ...
    const HOLIDAYS_FALLBACK = {
        '2025': [{date:'2025-01-01',name:'Tahun Baru Masehi'},{date:'2025-01-27',name:'Isra Mikraj'},{date:'2025-01-29',name:'Imlek 2576'},{date:'2025-03-14',name:'Nyepi'},{date:'2025-03-29',name:'Idul Fitri'},{date:'2025-03-30',name:'Idul Fitri'},{date:'2025-04-18',name:'Wafat Isa Al Masih'},{date:'2025-05-01',name:'Hari Buruh'},{date:'2025-05-12',name:'Waisak'},{date:'2025-05-29',name:'Kenaikan Isa'},{date:'2025-06-01',name:'Pancasila'},{date:'2025-06-06',name:'Idul Adha'},{date:'2025-06-27',name:'Tahun Baru Islam'},{date:'2025-08-17',name:'Kemerdekaan RI'},{date:'2025-09-05',name:'Maulid Nabi'},{date:'2025-12-25',name:'Natal'}],
        '2026': [{date:'2026-01-01',name:'Tahun Baru Masehi'},{date:'2026-01-16',name:'Isra Mikraj'},{date:'2026-02-16',name:'Imlek 2577'},{date:'2026-02-17',name:'Imlek 2577'},{date:'2026-03-18',name:'Nyepi'},{date:'2026-03-19',name:'Nyepi'},{date:'2026-03-20',name:'Idul Fitri'},{date:'2026-03-21',name:'Idul Fitri'},{date:'2026-03-22',name:'Idul Fitri'},{date:'2026-03-23',name:'Idul Fitri'},{date:'2026-03-24',name:'Idul Fitri'},{date:'2026-04-03',name:'Wafat Yesus Kristus'},{date:'2026-04-05',name:'Paskah'},{date:'2026-05-01',name:'Hari Buruh'},{date:'2026-05-14',name:'Kenaikan Yesus'},{date:'2026-05-15',name:'Kenaikan Yesus'},{date:'2026-05-27',name:'Idul Adha'},{date:'2026-05-28',name:'Idul Adha'},{date:'2026-05-31',name:'Waisak'},{date:'2026-06-01',name:'Pancasila'},{date:'2026-06-16',name:'Tahun Baru Islam'},{date:'2026-08-17',name:'Kemerdekaan RI'},{date:'2026-08-25',name:'Maulid Nabi'},{date:'2026-12-25',name:'Natal'}],
        '2027': [{date:'2027-01-01',name:'Tahun Baru Masehi'},{date:'2027-02-06',name:'Imlek 2578'},{date:'2027-03-10',name:'Idul Fitri'},{date:'2027-03-11',name:'Idul Fitri'},{date:'2027-03-22',name:'Nyepi'},{date:'2027-03-26',name:'Wafat Isa Al Masih'},{date:'2027-05-01',name:'Hari Buruh'},{date:'2027-05-06',name:'Kenaikan Isa'},{date:'2027-05-17',name:'Idul Adha'},{date:'2027-06-01',name:'Pancasila'},{date:'2027-06-07',name:'Tahun Baru Islam'},{date:'2027-08-17',name:'Kemerdekaan RI'},{date:'2027-12-25',name:'Natal'}]
    };

    function parseTanggalRange(tanggal, year, mIdx) {
        const parts = String(tanggal).split('-');
        const days  = parts.length === 2 ? Array.from({length: parseInt(parts[1])-parseInt(parts[0])+1}, (_,i) => parseInt(parts[0])+i) : [parseInt(parts[0])];
        ...
        try {
            if (!harikerjaCache[year]) {
                try {
                    ...
                    if (r.ok) { const j = await r.json(); if (j.code===200 && j.data) harikerjaCache[year] = j.data; }
                } catch(e) {}
            }
            if (harikerjaCache[year]) {
                ...
                if (found?.tanggal_merah?.length) {
                    national = [];
                    found.tanggal_merah.forEach(tm => parseTanggalRange(tm.tanggal, year, mIdx).forEach(d => national.push({date:d, name:tm.memperingati})));
                }
            }
            ...
            school = schResp.data || [];
        } catch(e) { national = getHolidaysFallback(year, parseInt(month)); }
    ...
    function isDateDisabled(dateStr, holidays) {
        ...
        if (day===0||day===6) return {locked:true, reason:'Libur Akhir Pekan'};
        const n = holidays.national.find(h => h.date===dateStr);
        const s = holidays.school.find(h => h.date===dateStr);
        if (n||s) return {locked:true, reason:(n||s).name};
    ...
    function setStatus(studentId, status) {
        ...
        if (card) {
            ...
            if (attendanceData[studentId]) { const a = card.querySelector(`.btn-${attendanceData[studentId]}`); if(a) a.classList.add('active'); }
            ...
            if (jamRow) {
                ...
                spans[0].textContent = jd; spans[0].className = `jam-value ${jd==='-'?'empty':''}`;
                spans[1].textContent = jp; spans[1].className = `jam-value ${jp==='-'?'empty':''}`;
            }
        }
        updateStats();
    }

    function markAll(status) { students.forEach(s => { attendanceData[s.id] = status; }); renderStudents(students); }

    function updateStats() {
        let hadir=0, sakit=0, izin=0, alpa=0;
        Object.values(attendanceData).forEach(s => { if(s==='hadir') hadir++; else if(s==='sakit') sakit++; else if(s==='izin') izin++; else if(s==='alpha') alpa++; });
    ...
    async function submitAttendance() {
        if (isLocked)  { showToast('Absensi dikunci untuk tanggal ini','error'); return; }
        if (!currentDate) { showToast('Tanggal belum dipilih','error'); return; }
        const entries = Object.entries(attendanceData);
        if (!entries.length) { showToast('Belum ada data kehadiran yang diisi','error'); return; }
        ...
        try {
            const records = entries.map(([sid, status]) => ({student_id:parseInt(sid), status, id:existingIds[sid]||null}));
        ...
        } catch(err) { console.error(err); showToast('Terjadi kesalahan saat menyimpan','error'); }
        finally {
            btn.disabled = false;
    ...
    // ── Helpers ────────────────────────────────────────────────────────────
    function getInitials(name) { const p=(name||'').trim().split(/\s+/); return p.length>=2?(p[0][0]+p[1][0]).toUpperCase():(name||'??').substring(0,2).toUpperCase(); }
    ...
    // ── Boot ───────────────────────────────────────────────────────────────
    document.addEventListener('DOMContentLoaded', () => { if (CLASS_ID) setDate(getTodayStr()); });
</script>
<?= $this->endSection() ?>